...
for phrase in possibilities:
	...
	for word in phrase.split(' '):
		...
		if len(word) != 3:
			...
			if word in workingWords:
				...
				for work in workingWords:
					...
					if matched == lowers:
						plaintext = phrase
						if work not in newPoss:
							newPoss.append(work)
							#print '[!]Appended newPoss:', work
							#for i in range(3):
							for letter in word:
								if letter.isupper():
									plaintext = plaintext.replace(letter, work[word.index(letter)])
							#print plaintext
							#raw_input()
							#possibilities.append(plaintext)
							#print plaintext
		...
		#print '[-]Current possibilities', newPoss
		#if word == 'and':
		#	raw_input()
	...
	for phrase in possibilities:
		...
		for word in phrase.split(' '):
			...
			if word.islower() and len(word) == 3:
				#print '[-]Is 3 lowercase'
				if word not in threeLtrWords:
					#print '[-]Invalid 3 letter word'
					try:
						possibilities.remove(phrase)
						#print '[-]Removed possibility'
						#raw_input()
						break
					except:
						#raw_input()
...
print 'Total Possibilities:', len(possibilities)
